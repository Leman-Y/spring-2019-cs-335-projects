 ...
 * Nassau St   &   Frankfort   St  at  SE  corner
 */
//Implemented list all portals of a station. I thought it was list literally all the portals
void SubwaySystem::list_all_portals(ostream & out, string station_name)
{
	...
	__ItemType find_object(station_name,-1);

	stations.find(find_object);

	int index_of_station=find_object.get_pos();
	cout<<index_of_station<<endl;
	list<int> list_of_portals=subway_array[index_of_station].portal_list();

   	cout<<"beginningof for loop"<<endl;
   	cout<<"portal list size "<<list_of_portals.size()<<endl;
    list <int> :: iterator it; 
    for(it = list_of_portals.begin(); it != list_of_portals.end(); ++it) 
   	{
   		cout<<"bruh"<<endl;
   		cout<<*it<<endl;
   		out<<subway_array[*it].portal_name();
   	} 
